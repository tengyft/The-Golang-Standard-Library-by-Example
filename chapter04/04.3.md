...
在go1.10.3之中，go已经支持单调时间了，Time结构变为:
```golang
type Time struct {
...
```
wall和ext编码了墙上时间的秒部分、纳秒部分，还编码了可选的单调时间(其以纳秒为单位)。

wall分三部分(从高位到低位): 1位标志位、33位秒部分、30位纳秒部分。其中纳秒范围为[0, 999999999]。

如果标志位hasMonotonic为0: 则33位秒部分全部为0，ext储存着自 1-1-1 00:00:00 以来的秒数，wall的剩余30位仍为纳秒部分。

如果标志位hasMonotonic为1: 则33位秒部分储存着自 1885-1-1 00:00:00 以来的秒数，wall的剩余30位仍为纳秒部分; ext为单调时间(以纳秒为单位，自进程开始时计算)。
